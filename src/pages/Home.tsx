...
const Home: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const count = isMobile ? 40 : isTablet ? 60 : 80;
  ...
  // Mouse parallax — desktop only (no-op on touch)
  useEffect(() => {
    if (showLoader || !isDesktop) return;
  ...
  useEffect(() => {
    if (!showLoader) setTimeout(() => setIsLoaded(true), 100);
  }, [showLoader]);
  ...
  const stats = [
    { icon: <Layers size={20} />, value: "10+", label: "Projects Built" },
    { icon: <Code2 size={20} />, value: "3rd Yr", label: "B.Tech CSE" },
    { icon: <Zap size={20} />, value: "2+", label: "Certifications" },
  ];

  if (showLoader) return <Loader />;

  return (
    <div style={{ position: "relative", minHeight: "100vh", overflow: "hidden", background: "#020408", fontFamily: "'DM Sans', sans-serif" }}>
      <style>{`
        ...
        :root {
          --cyan: #00d4ff; --purple: #7b2ff7; --pink: #ff006e;
          --green: #00ff88; --dark: #020408; --border: rgba(0,212,255,0.12);
        }
        * { box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
        ...
        @media (hover: none) { .btn-primary:hover { transform: none; box-shadow: none; } }
        ...
        .btn-outline:hover { border-color: var(--cyan); color: var(--cyan); background: rgba(0,212,255,0.06); transform: translateY(-2px); }
        @media (hover: none) { .btn-outline:hover { transform: none; } }
        ...
        .social-btn:hover { border-color: var(--cyan); color: var(--cyan); background: rgba(0,212,255,0.08); transform: translateY(-3px); }
        @media (hover: none) { .social-btn:hover { transform: none; } }
        ...
        .stat-card:hover { border-color: rgba(0,212,255,0.2); background: rgba(0,212,255,0.04); transform: translateY(-3px); }
        ...
        @keyframes scroll-bounce { 0%,100%{transform:translateY(0) translateX(-50%);opacity:.6} 50%{transform:translateY(8px) translateX(-50%);opacity:1} }
      ...
      {/* Grid Background */}
      <div className="grid-bg" style={{ position: "fixed", inset: 0, opacity: 0.4, zIndex: 0, pointerEvents: "none" }} />

      {/* Ambient orbs */}
      <div className="orb" style={{ width: 500, height: 500, top: -100, left: -100, background: "rgba(123,47,247,0.12)" }} />
      ...
      <section
        ...
        style={{
          ...
          transform: isDesktop ? `translate(${mousePos.x * -8}px, ${mousePos.y * -4}px)` : "none",
          transition: isDesktop ? "transform 0.3s ease" : "none",
        }}
      >
        <div style={{ width: "100%", maxWidth: 1280, margin: "0 auto", padding: `0 clamp(16px, 5vw, 48px)` }}>
          ...
          <div style={{
            ...
            <div>
              {/* Available badge */}
              <div className="delay-1" style={{
                display: "inline-flex", alignItems: "center", gap: 8,
                marginBottom: isMobile ? 18 : 24,
                padding: "6px 14px", borderRadius: "100px",
                background: "rgba(0,212,255,0.08)", border: "1px solid rgba(0,212,255,0.2)",
                fontSize: 12, color: "var(--cyan)", fontFamily: "'JetBrains Mono', monospace",
              }}>
              ...
              {/* Tech pills */}
              <div className="delay-3" style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: isMobile ? 24 : 32 }}>
              ...
              <div className="delay-4" style={{
                display: "flex",
                flexDirection: isMobile ? "column" : "row",
                gap: isMobile ? 10 : 14,
                marginBottom: isMobile ? 24 : 32,
              }}>
                <Link to="/projects" className="btn-primary" style={isMobile ? { justifyContent: "center" } : {}}>
                  <span>View Projects</span>
                  <ArrowRight size={16} />
                </Link>
                <a href="/resume.pdf" target="_blank" className="btn-outline" style={isMobile ? { justifyContent: "center" } : {}}>
                  <Download size={15} />
                  Download CV
                </a>
              </div>
              ...
              {/* Socials */}
              <div className="delay-5" style={{ display: "flex", gap: 10 }}>
                {[
            ...
            {/* ── RIGHT COLUMN — Visual (hidden on mobile, shown on tablet+ as simplified card) ── */}
            {!isMobile && (
              <div style={{ display: "flex", alignItems: "center", justifyContent: "center" }}>
                <div style={{ position: "relative", width: isTablet ? 300 : 380, height: isTablet ? 300 : 380 }}>
                  ...
                  <div className="hero-card" style={{
                    ...
                    <div style={{
                      width: isTablet ? 72 : 90, height: isTablet ? 72 : 90,
                      borderRadius: "50%",
                      background: "linear-gradient(135deg, #7b2ff7, #00d4ff)",
                      display: "flex", alignItems: "center", justifyContent: "center",
                      fontSize: isTablet ? 22 : 28, fontWeight: 800, color: "white",
                      fontFamily: "'Space Grotesk', sans-serif",
                      boxShadow: "0 0 40px rgba(123,47,247,0.4)",
                    }}>SM</div>
                    <div style={{ textAlign: "center" }}>
                      <div style={{ color: "white", fontWeight: 600, fontSize: isTablet ? 15 : 18, marginBottom: 4 }}>Shivam Modi</div>
                      <div style={{ color: "var(--cyan)", fontSize: 12, fontFamily: "'JetBrains Mono', monospace" }}>Full-Stack Developer</div>
                    </div>
                  ...
                  {/* Floating badges — desktop only, would overflow on tablet */}
                  {isDesktop && (
                    <>
          ...
          <div style={{
            ...
            {stats.map((s, i) => (
              <div key={i} className="stat-card">
                ...
                <div style={{ color: "white", fontWeight: 700, fontSize: isMobile ? 18 : 22, fontFamily: "'Space Grotesk', sans-serif" }}>{s.value}</div>
                <div style={{ color: "rgba(255,255,255,0.35)", fontSize: 10, letterSpacing: "0.05em", textTransform: "uppercase", textAlign: "center" }}>{s.label}</div>
              </div>
            ))}
          </div>
        </div>
