...
/* ── Robot SVG ─────────────────────────────────────────────────── */
const RobotSVG: React.FC<{ color: string; animated: boolean; size?: number }> = ({
  color,
  animated,
  size = 72,
}) => {
...
/* ── Typing Effect Hook ─────────────────────────────────────────── */
const useTyping = (text: string, active: boolean, speed = 22) => {
...
/* ── Robot Bubble ──────────────────────────────────────────────── */
// On desktop: floats left/right of the card (absolute, outside card flow)
// On mobile/tablet: renders inline below the card content (in-flow)
const RobotBubble: React.FC<{
  ...
  glow: string;
  position: "left" | "right";
  inline?: boolean; // mobile inline mode
}> = ({ visible, text, color, glow, position, inline = false }) => {
  const { displayed, done } = useTyping(text, visible, 20);

  if (inline) {
    // Inline version — sits below the card, full width, no absolute positioning
    return (
      <div
        style={{
      ...
      >
        ...
        >
          <div style={{ flexShrink: 0, marginTop: 2 }}>
            <RobotSVG color={color} animated={visible} size={52} />
          </div>
          <div style={{ flex: 1 }}>
            <div
              style={{
  ...
  // Desktop floating version (absolute, outside card)
  return (
    <div
      style={{
        position: "absolute",
...
/* ── Certificate Card ───────────────────────────────────────────── */
const CertCard: React.FC<{ cert: Cert; index: number; isDesktop: boolean }> = ({
  cert,
  index,
  isDesktop,
}) => {
      ...
      >
        ...
        {!active && (
          ...
          >
            ...
            {isDesktop ? "hover" : "tap"} for AI insight
          </div>
        )}

        {/* Inline robot bubble for mobile/tablet */}
        {!isDesktop && (
          <RobotBubble
            visible={active}
            text={cert.robotSays}
...
const CodingProfiles: React.FC = () => {
  const profiles = [
    ...
    {
      name: "GeeksforGeeks",
      url: "https://www.geeksforgeeks.org/user/smodi9kat/",
...
const Certifications: React.FC = () => {
  ...
  // Grid columns:
  // mobile  (<600px)  → 1 col, no side padding (robots inline)
  // tablet  (<960px)  → 1–2 col depending on card width, no side padding
  // desktop (≥960px)  → auto-fit ≥380px, side padding for floating robots
  const gridStyle: React.CSSProperties = isDesktop
    ? {
        display: "grid",
        gridTemplateColumns: "repeat(auto-fit, minmax(380px, 1fr))",
        gap: 20,
        padding: "0 90px", // room for floating robots
      }
    : bp === "tablet"
    ? {
        display: "grid",
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            {isDesktop
              ...
              : "Tap any card to summon the AI assistant — it'll explain what each achievement means."}
          </p>
        </div>
        ...
        <div style={gridStyle}>
          {CERTS.map((cert, i) => (
            <CertCard key={cert.id} cert={cert} index={i} isDesktop={isDesktop} />
          ))}
        </div>
